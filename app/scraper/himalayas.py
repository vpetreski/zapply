...
class HimalayasScraper(BaseScraper):
        ...
        async with httpx.AsyncClient(
            ...
            while offset < self.MAX_PAGES * self.MAX_PER_PAGE and not found_old_job:
                ...
                if not data:
                    ...
                    if progress_callback:
                        await progress_callback(f"Failed to fetch page at offset {offset}", "error")
                    break
                ...
                if not page_jobs:
                    ...
                    if progress_callback:
                        await progress_callback("No more jobs to scan", "info")
                    break
                ...
                for job in page_jobs:
                    ...
                    if pub_date < cutoff_date:
                        ...
                        if consecutive_old_jobs > self.MAX_CONSECUTIVE_OLD_JOBS:
                            ...
                            if progress_callback:
                                await progress_callback(f"Found {consecutive_old_jobs} old jobs, stopping scan", "info")
                            found_old_job = True
                            break
                        continue
                    ...
                    if progress_callback:
                        await progress_callback(f"Found: {title} @ {company}", "success")
    ...
    def _normalize_job(self, job: dict[str, Any], slug: str) -> dict[str, Any]:
        ...
        return {
            ...
            "description": self._html_to_text(job.get("description", "")),
